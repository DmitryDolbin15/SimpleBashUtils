...
do
                for
                  p in "${pattern1[@]}"
                       ...
                       "${pattern3[@]}";
do
  echo grep $i $p $j grep $i $p $j > grep.test.txt./ s21_grep $i $p $j >
      s21_grep.test.txt((COUNTER++)) if cmp - s
          "grep.test.txt"
          "s21_grep.test.txt";
then((SUCCESS++)) echo - e "${Green}Same${NC}" else echo grep $i $p $j
    >> error.log

           ((FAIL++)) echo
           -
           e "${RED}Differ${NC}" fi rm grep.test.txt rm s21_grep.test
               .txt done done done

           echo "--------" echo
           -
           e
           "Count: $COUNTER : ${Green}Success:$SUCCESS${NC} -- "
           "${RED}Fail:$FAIL${NC}"